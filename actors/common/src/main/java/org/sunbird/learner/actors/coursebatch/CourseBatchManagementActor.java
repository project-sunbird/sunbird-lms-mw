...
public class CourseBatchManagementActor extends BaseActor {
  ...
  private void updateCourseBatch(Request actorMessage) {
    ...
    targetObject = TelemetryUtil.generateTargetObject((String) request.get(JsonKey.ID), JsonKey.BATCH, JsonKey.UPDATE,
        null);
    ...
    if (PropertiesCache.getInstance().getProperty(JsonKey.COURSE_BATCH_NOTIFICATIONS_ACTIVE) != null && (Boolean
      ...
      batchNotificationMap.put(BackgroundOperations.OLD.name(), oldBatch);
      batchNotification.setRequest(batchNotificationMap);
      batchOperationNotifier(batchNotification);
    }
  ...
  private void checkBatchStatus(CourseBatch courseBatch) {
    ProjectLogger.log("CourseBatchManagementActor:checkBatchStatus batch staus is :" + courseBatch.getStatus(),
    ...
    if (ProjectUtil.ProgressStatus.COMPLETED.getValue() == courseBatch.getStatus()) {
      throw new ProjectCommonException(ResponseCode.courseBatchEndDateError.getErrorCode(),
          ResponseCode.courseBatchEndDateError.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
    }
  }
  ...
  private CourseBatch getUpdateCourseBatch(Map<String, Object> request) {
    ...
    courseBatch.setEnrollmentType(
        getEnrollmentType((String) request.get(JsonKey.ENROLLMENT_TYPE), courseBatch.getEnrollmentType()));
    courseBatch.setCreatedFor(getUpdatedCreatedFor((List<String>) request.get(JsonKey.COURSE_CREATED_FOR),
        courseBatch.getEnrollmentType(), courseBatch.getCreatedFor()));
    courseBatch.setHashTagId(
        getHashTagId((String) request.get(JsonKey.HASHTAGID), JsonKey.UPDATE, (String) request.get(JsonKey.ID), ""));

    if (request.containsKey(JsonKey.NAME))
      courseBatch.setName((String) request.get(JsonKey.NAME));

    if (request.containsKey(JsonKey.DESCRIPTION))
      courseBatch.setDescription((String) request.get(JsonKey.DESCRIPTION));

    if (request.containsKey(JsonKey.MENTORS) && !((List<String>) request.get(JsonKey.MENTORS)).isEmpty())
  ...
  private String getEnrollmentType(String requestEnrollmentType, String dbEnrollmentType) {
    if (requestEnrollmentType != null)
      return requestEnrollmentType;
  ...
  private void addUserCourseBatch(Request actorMessage) {
    ...
    if (ProjectUtil.isNull(courseBatchObject.get(JsonKey.ENROLLMENT_TYPE))
      ...
      throw new ProjectCommonException(ResponseCode.enrollmentTypeValidation.getErrorCode(),
          ResponseCode.enrollmentTypeValidation.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
    }
    ...
    if (ProjectUtil.isNull(courseBatchObject.get(JsonKey.COURSE_CREATED_FOR))
      ...
      throw new ProjectCommonException(ResponseCode.courseCreatedForIsNull.getErrorCode(),
          ResponseCode.courseCreatedForIsNull.getErrorMessage(), ResponseCode.RESOURCE_NOT_FOUND.getResponseCode());
    }
    ...
    if (StringUtils.isBlank(batchCreator)) {
      throw new ProjectCommonException(ResponseCode.invalidCourseCreatorId.getErrorCode(),
          ResponseCode.invalidCourseCreatorId.getErrorMessage(), ResponseCode.RESOURCE_NOT_FOUND.getResponseCode());
    }
    ...
    for (String userId : userIds) {
      if (!(participants.containsKey(userId))) {
        ...
        participants.put(userId, userCoursesService.enroll(batchId, (String) courseBatchObject.get(JsonKey.COURSE_ID),
    ...
    try {
      ...
      ProjectLogger.log(
          "CourseBatchManagementActor:addUserCourseBatch: Exception occurred with error message = " + ex.getMessage(),
          ex);
    }
  }
  ...
  private void getCourseBatch(Request actorMessage) {
    Map<String, Object> result = ElasticSearchUtil.getDataByIdentifier(ProjectUtil.EsIndex.sunbird.getIndexName(),
        ProjectUtil.EsType.course.getTypeName(), (String) actorMessage.getContext().get(JsonKey.BATCH_ID));
    Response response = new Response();
    if (null != result) {
  ...
  private void syncCourseBatchForeground(String uniqueId, Map<String, Object> req) {
    ProjectLogger.log("CourseBatchManagementActor: syncCourseBatchForeground called for course batch ID = " + uniqueId,
        LoggerEnum.INFO.name());
    ...
    ProjectLogger.log("CourseBatchManagementActor::syncCourseBatchForeground: Sync response for course batch ID = "
        + uniqueId + " received response = " + esResponse, LoggerEnum.INFO.name());
  }
  ...
  private int setCourseBatchStatus(String startDate) {
    try {
      ...
      ProjectLogger.log(
          "CourseBatchManagementActor:setCourseBatchStatus: Exception occurred with error message = " + e.getMessage(),
          e);
    }
    return ProgressStatus.NOT_STARTED.getValue();
  }
  ...
  private void syncCourseBatchBackground(Map<String, Object> req, String operation) {
    ProjectLogger.log("CourseBatchManagementActor: syncCourseBatchBackground called", LoggerEnum.INFO.name());
    Request request = new Request();
    ...
    try {
      ...
      ProjectLogger.log("CourseBatchManagementActor:syncCourseBatchBackground: Exception occurred with error message = "
          + ex.getMessage(), ex);
    }
  }
    ...
    if (mentors != null) {
      ...
      for (String userId : mentors) {
        ...
        if (!batchCreatorRootOrgId.equals(mentorRootOrgId)) {
          throw new ProjectCommonException(ResponseCode.userNotAssociatedToRootOrg.getErrorCode(),
              ResponseCode.userNotAssociatedToRootOrg.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode(),
              userId);
        }
  ...
  private Map<String, Boolean> getParticipantsMap(List<String> participants, CourseBatch courseBatchObject) {
    ...
    for (String userId : participants) {
      if (!(dbParticipants.containsKey(userId))) {
        finalParticipants.put(userId, userCoursesService.enroll(batchId, courseBatchObject.getCourseId(), userId,
    ...
    for (String userId : participants) {
      if (!participantWithRootOrgIds.containsKey(userId)
        ...
        throw new ProjectCommonException(ResponseCode.userNotAssociatedToRootOrg.getErrorCode(),
            ...
            ResponseCode.RESOURCE_NOT_FOUND.getResponseCode(), userId);
      }
    }
  }

  private void removeParticipants(Map<String, Boolean> removedParticipant, String batchId, String courseId) {
    removedParticipant.forEach((userId, aBoolean) -> {
  ...
  private void validateCourseBatchData(CourseBatch courseBatchObject) {
    if (ProjectUtil.isNull(courseBatchObject.getCreatedFor()) || courseBatchObject.getCreatedFor().isEmpty()) {
      throw new ProjectCommonException(ResponseCode.courseCreatedForIsNull.getErrorCode(),
          ResponseCode.courseCreatedForIsNull.getErrorMessage(), ResponseCode.RESOURCE_NOT_FOUND.getResponseCode());
    }
  ...
  private List<String> getUpdatedCreatedFor(List<String> createdFor, String enrolmentType,
      List<String> dbValueCreatedFor) {
    if (createdFor != null) {
  ...
  private void updateCourseBatchDate(CourseBatch courseBatch, Map<String, Object> req) {
    ...
    courseBatch
        .setStartDate(requestedStartDate != null ? (String) req.get(JsonKey.START_DATE) : courseBatch.getStartDate());
    courseBatch.setEndDate(requestedEndDate != null ? (String) req.get(JsonKey.END_DATE) : courseBatch.getEndDate());
  }
  ...
  private Map<String, String> getRootOrgForMultipleUsers(List<String> userIds) {
    ...
    Map<String, Object> result = ElasticSearchUtil.complexSearch(searchDTO, ProjectUtil.EsIndex.sunbird.getIndexName(),
  ...
  private String getHashTagId(String hasTagId, String operation, String id, String uniqueId) {
    if (hasTagId != null)
      return validateHashTagId(hasTagId, operation, id);
    return uniqueId;
  }
    ...
    if (opType.equalsIgnoreCase(JsonKey.CREATE)) {
      if (!dataMapList.isEmpty()) {
        throw new ProjectCommonException(ResponseCode.invalidHashTagId.getErrorCode(),
            ResponseCode.invalidHashTagId.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
      }
    ...
    if (startDate != null) {
      try {
        ...
        throw new ProjectCommonException(ResponseCode.dateFormatError.getErrorCode(),
            ResponseCode.dateFormatError.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
      }
    } else {
      throw new ProjectCommonException(ResponseCode.courseBatchStartDateRequired.getErrorCode(),
          ResponseCode.courseBatchStartDateRequired.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
    }
  }

  private void validateBatchStartAndEndDate(Date existingStartDate, Date existingEndDate, Date requestedStartDate,
      Date requestedEndDate, Date todayDate) {
    ...
    ProjectLogger.log(
        "existingStartDate, existingEndDate, requestedStartDate, requestedEndDate, todaydate" + existingStartDate + ","
            + existingEndDate + "," + requestedStartDate + "," + requestedEndDate + "," + todayDate,
        LoggerEnum.INFO.name());
    ...
    if ((existingStartDate.before(todayDate) || existingStartDate.equals(todayDate))
      ...
      throw new ProjectCommonException(ResponseCode.invalidBatchStartDateError.getErrorCode(),
          ResponseCode.invalidBatchStartDateError.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
    }
    ...
    if (endDate != null && startDate.after(endDate)) {
      throw new ProjectCommonException(ResponseCode.invalidBatchEndDateError.getErrorCode(),
          ResponseCode.invalidBatchEndDateError.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
    }
  ...
  private Date getDate(String key, SimpleDateFormat format, Map<String, Object> map) {
    try {
      ...
      ProjectLogger.log("CourseBatchManagementActor:getDate: Exception occurred with message = " + e.getMessage(), e);
    }
    return null;
  }

  private boolean isOrgValid(String orgId) {
    Map<String, Object> resp = ElasticSearchUtil.getDataByIdentifier(ProjectUtil.EsIndex.sunbird.getIndexName(),
        ProjectUtil.EsType.organisation.getTypeName(), orgId);
    if (resp != null && resp.size() > 0) {
      ProjectLogger.log("CourseBatchManagementActor:isOrgValid: Organisation found in ES with id = " + orgId);
      return true;
    }
    ProjectLogger.log("CourseBatchManagementActor:isOrgValid: Organisation NOT found in ES with id = " + orgId);
    return false;
  }

  private Map<String, Object> getContentDetails(String courseId, Map<String, String> headers) {
    Map<String, Object> ekStepContent = CourseEnrollmentActor.getCourseObjectFromEkStep(courseId, headers);
    if (null == ekStepContent || ekStepContent.size() == 0) {
      ProjectLogger.log("CourseBatchManagementActor:getEkStepContent: Not found course for ID = " + courseId,
          LoggerEnum.INFO.name());
      throw new ProjectCommonException(ResponseCode.invalidCourseId.getErrorCode(),
          ResponseCode.invalidCourseId.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
    }
