...
public class AuditLogServiceImplTest {
	...
	public void testACreateUserAuditLog() {
		...
		AuditOperation op = (AuditOperation) Util.auditLogUrlMap.get(ActorOperations.CREATE_USER.getValue());
		...
		List<Object> auditRecordList = (List<Object>) ((Map<String, Object>) responseObj.getResult()
				.get(JsonKey.RESPONSE)).get(JsonKey.CONTENT);
	...
	public void testCreateBatchAuditLog() {
		...
		AuditOperation op = (AuditOperation) Util.auditLogUrlMap.get(ActorOperations.CREATE_BATCH.getValue());
	...
	public static void delete() {
		for (String auditId : auditIds) {
			ElasticSearchUtil.removeData(EsIndex.sunbirdDataAudit.getIndexName(), EsType.history.getTypeName(),
					auditId);
		}
	}
