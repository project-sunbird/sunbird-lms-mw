...
public class EsSyncActor extends UntypedAbstractActor {
	...
	private EncryptionService service = org.sunbird.common.models.util.datasecurity.impl.ServiceFactory
			.getEncryptionServiceInstance(null);
	private DataMaskingService maskingService = org.sunbird.common.models.util.datasecurity.impl.ServiceFactory
	...
	private void syncData(Request message) {
		...
		if (null != objectIds && !objectIds.isEmpty()) {
			...
			Response response = cassandraOperation.getRecordsByProperty(dbInfo.getKeySpace(), dbInfo.getTableName(),
					JsonKey.ID, objectIds);
		...
		ProjectLogger.log(
				"total time taken to sync db data for " + objectType + " to Elastic search " + elapsedTime + " ms.");
	}
	...
	private Map<String, Object> getOrgDetails(Entry<String, Object> entry) {
		...
		if (orgMap.containsKey(JsonKey.ADDRESS_ID)
			...
			orgMap.put(JsonKey.ADDRESS,
					getDetailsById(Util.dbInfoMap.get(JsonKey.ADDRESS_DB), (String) orgMap.get(JsonKey.ADDRESS_ID)));
		}
	...
	private Map<String, Object> getUserDetails(Entry<String, Object> entry) {
		...
		List<Map<String, Object>> eduMap = getDetails(Util.dbInfoMap.get(JsonKey.EDUCATION_DB), userId,
				JsonKey.USER_ID);
		for (Map<String, Object> map : eduMap) {
			if (map.containsKey(JsonKey.ADDRESS_ID)
				...
				map.put(JsonKey.ADDRESS,
						getDetailsById(Util.dbInfoMap.get(JsonKey.ADDRESS_DB), (String) map.get(JsonKey.ADDRESS_ID)));
			}
		}
		userMap.put(JsonKey.EDUCATION, eduMap);
		ProjectLogger.log("fetching user job profile data started");
		List<Map<String, Object>> jobMap = getDetails(Util.dbInfoMap.get(JsonKey.JOB_PROFILE_DB), userId,
				JsonKey.USER_ID);
		...
		List<Map<String, Object>> badgesMap = getDetails(Util.dbInfoMap.get(JsonKey.USER_BADGES_DB), userId,
				JsonKey.RECEIVER_ID);
		...
		Map<String, String> profileVisibility = (Map<String, String>) userMap.get(JsonKey.PROFILE_VISIBILITY);
	...
	private Map<String, Object> getDetailsById(DbInfo dbInfo, String userId) {
		try {
			...
			return ((((List<Map<String, Object>>) response.get(JsonKey.RESPONSE)).isEmpty()) ? new HashMap<>()
