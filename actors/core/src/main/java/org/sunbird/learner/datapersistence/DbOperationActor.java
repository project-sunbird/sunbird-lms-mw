...
@ActorConfig(tasks = {"createData", "updateData", "deleteData", "readData", "readAllData",
        "searchData", "getMetrics"}, asyncTasks = {})
public class DbOperationActor extends BaseActor {
    ...
    public void onReceive(Request actorMessage) throws Throwable {
        ...
        if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.CREATE_DATA.getValue())) {
            ...
            delete(actorMessage);
                ...
                .equalsIgnoreCase(ActorOperations.READ_DATA.getValue())) {
            read(actorMessage);
        } else if (actorMessage.getOperation()
        ...
        try {
            ...
            if (!StringUtils.isBlank((String) reqObj.getRequest().get(ENTITY_NAME))) {
                ...
                if (null != reqObj.getRequest().get(JsonKey.FILTERS)) {
                    validateRequestData(
                            (Map<String, Object>) reqObj.getRequest().get(JsonKey.FILTERS));
                }
    ...
    private void delete(Request reqObj) {
        try {
            ...
            Map<String, Object> targetObject =
                    TelemetryUtil.generateTargetObject((String) reqObj.getRequest().get(JsonKey.ID),
                            (String) reqObj.getRequest().get(ENTITY_NAME), JsonKey.DELETE, null);
            TelemetryUtil.telemetryProcessingCall(reqObj.getRequest(), targetObject,
                    correlatedObject);
        } catch (Exception ex) {
            sender().tell(ex, self());
        }
    ...
    private void update(Request reqObj) {
        try {
            ...
            if (!StringUtils.isBlank((String) reqObj.getRequest().get(ENTITY_NAME))
                    && ((boolean) reqObj.getRequest().get(INDEXED))) {
                esResult = updateDataToElastic(ES_INDEX_NAME,
                        (String) reqObj.getRequest().get(ENTITY_NAME),
                ...
                if (esResult) {
                    ...
                    if (!((String) response.get(JsonKey.RESPONSE)).equals(JsonKey.SUCCESS)) {
                        deleteDataFromElastic(ES_INDEX_NAME,
                                (String) reqObj.getRequest().get(ENTITY_NAME),
            ...
            sender().tell(response, self());
            Map<String, Object> targetObject =
                    TelemetryUtil.generateTargetObject((String) payload.get(JsonKey.ID),
                            (String) reqObj.getRequest().get(ENTITY_NAME), JsonKey.UPDATE, null);
            TelemetryUtil.telemetryProcessingCall(
                    (Map<String, Object>) reqObj.getRequest().get(PAYLOAD), targetObject,
                    correlatedObject);
        } catch (Exception ex) {
            sender().tell(ex, self());
        }
    ...
    private void create(Request reqObj) {
        try {
            ...
            if (((String) response.get(JsonKey.RESPONSE)).equals(JsonKey.SUCCESS)
                ...
                boolean esResult = false;
                if (!StringUtils.isBlank((String) reqObj.getRequest().get(ENTITY_NAME))) {
                    esResult = insertDataToElastic(ES_INDEX_NAME,
                            (String) reqObj.getRequest().get(ENTITY_NAME),
                            (String) payload.get(JsonKey.ID), payload);
                    if (!esResult) {
                        deleteRecord(JsonKey.SUNBIRD_PLUGIN,
                                (String) reqObj.getRequest().get(ENTITY_NAME),
                                (String) payload.get(JsonKey.ID));
                        throw new ProjectCommonException(ResponseCode.esUpdateFailed.getErrorCode(),
                                ResponseCode.esUpdateFailed.getErrorMessage(),
            ...
            Map<String, Object> targetObject =
                    TelemetryUtil.generateTargetObject((String) payload.get(JsonKey.ID),
                            (String) reqObj.getRequest().get(ENTITY_NAME), JsonKey.CREATE, null);
            TelemetryUtil.telemetryProcessingCall(
                    (Map<String, Object>) reqObj.getRequest().get(PAYLOAD), targetObject,
                    correlatedObject);
        } catch (Exception ex) {
    ...
    private boolean insertDataToElastic(String index, String type, String identifier,
        ...
        ProjectLogger.log("Getting ES save response for type , identiofier==" + type + "  "
                + identifier + "  " + response);
