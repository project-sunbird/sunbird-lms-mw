...
public class CoursePublishedUpdate implements Job {
      ...
      if (null != ekStepResult && !ekStepResult.isEmpty()) {
        ...
        for (String courseId : ekStepResult) {
          try {
            ...
            Response response =
                cassandraOperation.getRecordsByProperty(courseBatchDBInfo.getKeySpace(),
      ...
      if ((int) batch.get(JsonKey.STATUS) != ProjectUtil.ProgressStatus.COMPLETED.getValue()) {
        ...
        for (Map.Entry<String, Boolean> entry : participants.entrySet()) {
          if (!entry.getValue()) {
            ...
            userCourses.put(JsonKey.COURSE_LOGO_URL,
                additionalCourseInfo.get(JsonKey.COURSE_LOGO_URL));
            ...
            if (!ProjectUtil
                .isStringNullOREmpty(additionalCourseInfo.get(JsonKey.LEAF_NODE_COUNT))) {
            ...
            try {
              ...
              // TODO: for some reason, ES indexing is failing with Timestamp value. need to check
              // and correct it.
  ...
  private void updateCourseStatusTable(List<String> ekStepResult) {
    ...
    for (String courseId : ekStepResult) {
      ...
      try {
        cassandraOperation.updateRecord(coursePublishDBInfo.getKeySpace(),
            coursePublishDBInfo.getTableName(), map);
