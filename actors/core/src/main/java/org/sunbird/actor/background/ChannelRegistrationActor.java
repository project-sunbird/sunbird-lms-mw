...
@RouterConfig(request = {}, bgRequest = { "registerChannel" })
public class ChannelRegistrationActor extends BaseActor {
	...
	private void processChannelReg(List<String> ekstepChannelList, List<Map<String, Object>> sunbirdChannelList) {
		Boolean bool = true;
	...
	private void updateSystemSettingTable(Boolean bool) {
		...
		ProjectLogger
				.log("Upsert operation result for channel reg status =  " + response.getResult().get(JsonKey.RESPONSE));
	...
	private List<String> getEkstepChannelList() {
		...
		try {
			...
			response = HttpUtil.sendPostRequest(
					(ekStepBaseUrl + PropertiesCache.getInstance().getProperty(JsonKey.EKSTEP_GET_CHANNEL_LIST)),
					reqString, headerMap);
			jObject = new JSONObject(response);
			data = jObject.getJSONObject(JsonKey.RESULT);
			ProjectLogger.log(
					"Total number of content fetched from Ekstep while getting List of channel : " + data.get("count"));
