...
@ActorConfig(
  tasks = {"searchAuditLog", "processAuditLog"},
  asyncTasks = {"searchAuditLog", "processAuditLog"}
)
public class ActorAuditLogServiceImpl extends BaseActor implements AuditLogService {
  ...
  @SuppressWarnings("unchecked")
  protected Map<String, Object> createAuditLogReqMap(
      AuditOperation op, Request message, Response result) {
  ...
  public void save(Map<String, Object> requestedData) {
    ElasticSearchUtil.createData(
        EsIndex.sunbirdDataAudit.getIndexName(),
        EsType.history.getTypeName(),
        ProjectUtil.getUniqueIdFromTimestamp(1),
        requestedData);
  }
  ...
  private void searchAuditHistory(Request actorMessage) {
    Map<String, Object> filters =
        (Map<String, Object>) actorMessage.getRequest().get(JsonKey.FILTERS);
    ...
    Map<String, Object> result =
        ElasticSearchUtil.complexSearch(
            searchDTO, EsIndex.sunbirdDataAudit.getIndexName(), EsType.history.getTypeName());
    // Decrypt user data
      ...
      if (((String) dataMap.get(JsonKey.OBJECT_TYPE)).equalsIgnoreCase(JsonKey.USER)) {
        ...
        if ((null != (List<Map<String, Object>>) relationList.get(JsonKey.ADDRESS))
          ...
          UserUtility.decryptUserAddressData(
              (List<Map<String, Object>>) relationList.get(JsonKey.ADDRESS));
        }
      }
    }
