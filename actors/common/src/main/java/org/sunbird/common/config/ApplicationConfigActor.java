          ...
          for (Map<String, Object> result : values) {
            ...
            if (count > 1) {
              throw new ProjectCommonException(
                  ResponseCode.duplicatePhoneData.getErrorCode(),
                  MessageFormat.format(
                      ResponseCode.duplicatePhoneData.getErrorMessage(), JsonKey.PHONE),
      ...
      if (null != esResponse) {
        ...
        if (null != facetsRes && !facetsRes.isEmpty()) {
          ...
          for (Map<String, Object> result : values) {
            ...
            if (count > 1) {
              throw new ProjectCommonException(
                  ResponseCode.duplicateEmailData.getErrorCode(),
                  MessageFormat.format(
                      ResponseCode.duplicateEmailData.getErrorMessage(), JsonKey.EMAIL),
                  ResponseCode.CLIENT_ERROR.getResponseCode());
            }
          }
        }
