...
public class PageManagementActorTest {
  ...
  public static void deletePageAndSection() {
    ...
    if (pageIdWithOrg != null) {
      operation.deleteRecord(pageMgmntDbInfo.getKeySpace(), pageMgmntDbInfo.getTableName(),
          pageIdWithOrg);
    }
    ...
    if (sectionId != null) {
      operation.deleteRecord(pageSectionDbInfo.getKeySpace(), pageSectionDbInfo.getTableName(),
          sectionId);
    }
     ...
     * result.get(JsonKey.RESPONSE); if (!(list.isEmpty())) { for (Map<String, Object> res : list) {
     * String id = (String) res.get(JsonKey.ID); System.out.println("ID is " + id);
     * operation.deleteRecord(pageMgmntDbInfo.getKeySpace(), pageMgmntDbInfo.getTableName(), id);
     *//*
       * ElasticSearchUtil.removeData(ProjectUtil.EsIndex.sunbird.getIndexName(),
       * EsType.user.getTypeName(), id);
