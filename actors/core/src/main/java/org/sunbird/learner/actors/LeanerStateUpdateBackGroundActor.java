...
@RouterConfig(request = {}, bgRequest = { "updateLearnerState" })
public class LeanerStateUpdateBackGroundActor extends BaseActor {
			...
			for (Map<String, Object> map : contentList) {
				...
				if (map.get(JsonKey.COURSE_ID) != null) {
					...
					if (temp.containsKey(primary)) {
						...
						innerMap.put(JsonKey.CONTENT, getLatestContent(
								(Map<String, Object>) ((Map<String, Object>) temp.get(primary)).get(JsonKey.CONTENT),
								map));
	...
	private void updateCourse(Map<String, Object> temp, Map<String, Integer> contentStateInfo) {
		...
		for (Map.Entry<String, Object> entry : temp.entrySet()) {
			...
			List<Map<String, Object>> courseList = (List<Map<String, Object>>) response.getResult()
			...
			if (null != courseList && !courseList.isEmpty()) {
				...
				updateDb.put(JsonKey.LAST_READ_CONTENT_STATUS,
						(contentStateInfo.get((String) ((Map<String, Object>) value.get("content")).get(JsonKey.ID))));
				try {
	...
	/**
	 * This method will combined map values with delimiter and create an encrypted
	 * key.
	 *
	 * @param req
	 *            Map<String , Object>
