...
public class FuzzySearchManager {
    ...
    private FuzzySearchManager(Map<String, Object> fuzzySearchMap, List<Map<String, Object>> searchMap) {
        this.fuzzySearchMap = fuzzySearchMap;
        this.searchMap = searchMap;
    }

    public static FuzzySearchManager getInstance(Map<String, Object> fuzzySearchMap, List<Map<String, Object>> searchMap) {
    ...
    private void validateKeyInFuzzyMap(String key) {
        ...
        if (!resultMap.keySet().contains(key)) {
            ProjectLogger.log(String.format("%s:%s:key not found in searchMap %s",this.getClass().getSimpleName(),"getFuzzyAttributeFromMap",key), LoggerEnum.ERROR.name());
    ...
    private Map<String, String> getFuzzyAttributeFromMap(String key) {
        ...
        searchMap.stream().forEach(result -> {
            ...
            attributesValueMap.put((String) resultMap.get(JsonKey.ID), (String) resultMap.get(key));

        });
        ProjectLogger.log(String.format("%s:%s:the prepared Map for fuzzy search  %s",this.getClass().getSimpleName(),"getFuzzyAttributeFromMap",Collections.singleton(attributesValueMap.toString())), LoggerEnum.INFO.name());
    ...
    private List<Map<String, Object>> fetchResultSetFromSearchMap(HashSet<String> fuzzyResultSet) {
        ...
        ProjectLogger.log(String.format("%s:%s:returning only fuzzy matched result, the size of List of map is   %s",this.getClass().getSimpleName(),"fetchResultSetFromSearchMap",responseList.size()), LoggerEnum.INFO.name());
        return responseList;
    }
