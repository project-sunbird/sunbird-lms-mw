...
public class OrganisationManagementActor extends BaseActor {
	...
	public void onReceive(Request request) throws Throwable {
		...
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.JOIN_USER_ORGANISATION.getValue())) {
			joinUserOrganisation(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.APPROVE_USER_ORGANISATION.getValue())) {
			approveUserOrg(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.REJECT_USER_ORGANISATION.getValue())) {
			rejectUserOrg(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.ADD_MEMBER_ORGANISATION.getValue())) {
			addMemberOrganisation(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.REMOVE_MEMBER_ORGANISATION.getValue())) {
			removeMemberOrganisation(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.GET_ORG_TYPE_LIST.getValue())) {
			getOrgTypeList();
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.CREATE_ORG_TYPE.getValue())) {
			createOrgType(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.UPDATE_ORG_TYPE.getValue())) {
	...
	private void updateOrgType(Request actorMessage) {
		...
		try {
			...
			targetObject = TelemetryUtil.generateTargetObject((String) request.get(JsonKey.ID), "organisationType",
					JsonKey.UPDATE, null);
			...
			new Thread() {
				...
				public void run() {
					if (((String) response.get(JsonKey.RESPONSE)).equalsIgnoreCase(JsonKey.SUCCESS)) {
						DataCacheHandler.getOrgTypeMap().put(((String) request.get(JsonKey.NAME)).toLowerCase(),
	...
	private void createOrgType(Request actorMessage) {
		...
		try {
			...
			targetObject = TelemetryUtil.generateTargetObject(id, "organisationType", JsonKey.CREATE, null);
			TelemetryUtil.telemetryProcessingCall(actorMessage.getRequest(), targetObject, correlatedObject);
	...
	private void createOrg(Request actorMessage) {
		...
		try {
			...
			targetObject = TelemetryUtil.generateTargetObject(uniqueId, JsonKey.ORGANISATION, JsonKey.CREATE, null);
	...
	private String validateLocationId(String locId) {
		...
		Response response = cassandraOperation.getRecordById(geoLocDbInfo.getKeySpace(), geoLocDbInfo.getTableName(),
				locId);
		List<Map<String, Object>> list = (List<Map<String, Object>>) response.get(JsonKey.RESPONSE);
		if (!list.isEmpty()) {
			Map<String, Object> map = list.get(0);
			...
			throw new ProjectCommonException(ResponseCode.invalidLocationId.getErrorCode(),
					ResponseCode.invalidLocationId.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
		}
	}
		...
		if (!StringUtils.isBlank(DataCacheHandler.getOrgTypeMap().get(orgType.toLowerCase()))) {
			...
			if (null == orgTypeId) {
				throw new ProjectCommonException(ResponseCode.invalidOrgType.getErrorCode(),
						ResponseCode.invalidOrgType.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
			}
		}
		return orgTypeId;
	}
	...
	private void approveOrg(Request actorMessage) {
		...
		try {
			...
			Response response = cassandraOperation.updateRecord(orgDbInfo.getKeySpace(), orgDbInfo.getTableName(),
					updateOrgDBO);
	...
	private void updateOrgData(Request actorMessage) {
		...
		try {
			...
			if (null != orgDBO.get(JsonKey.IS_ROOT_ORG) && (boolean) orgDBO.get(JsonKey.IS_ROOT_ORG)) {
				...
				if (null != updateOrgDBOChannel && null != channel && !(updateOrgDBOChannel.equals(channel))) {
			...
			targetObject = TelemetryUtil.generateTargetObject((String) orgDBO.get(JsonKey.ID), JsonKey.ORGANISATION,
					JsonKey.UPDATE, null);
			TelemetryUtil.telemetryProcessingCall(actorMessage.getRequest(), targetObject, correlatedObject);
	...
	private void removeMemberOrganisation(Request actorMessage) {
		...
		if (list.isEmpty()) {
			...
			sender().tell(response, self());
			Map<String, Object> targetObject = TelemetryUtil.generateTargetObject(userId, JsonKey.USER, JsonKey.UPDATE,
					null);
			TelemetryUtil.generateCorrelatedObject(userId, "userOrgMember", null, correlatedObject);
			TelemetryUtil.generateCorrelatedObject(orgId, "orgUserMember", null, correlatedObject);
	...
	/**
	 * Inserts the organization to organization relation is not exists, else Updates
	 * the organization to organization relation
	 */
	public void upsertOrgMap(String orgId, String parentOrgId, String rootOrgId, int env, String relation) {
		...
		cassandraOperation.insertRecord(orgMapDbInfo.getKeySpace(), orgMapDbInfo.getTableName(), orgMap);
	}
	...
	public Boolean isValidParent(String parentId) {
		...
		throw new ProjectCommonException(ResponseCode.invalidParentId.getErrorCode(),
				ResponseCode.invalidParentId.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
	}
	...
	public Boolean isChildOf(String parentId, String childId) {
		...
		Response result = cassandraOperation.getRecordsByProperties(userdbInfo.getKeySpace(), userdbInfo.getTableName(),
				properties);
		List<Map<String, Object>> list = (List<Map<String, Object>>) result.get(JsonKey.RESPONSE);
		return (!(list.isEmpty()));
	}
	...
	/**
	 * validates if the organization and parent organization has the same root
	 * organization else throws error
	 */
	@SuppressWarnings("unchecked")
	...
	private boolean validateOrgRequestForMembers(Map<String, Object> req) {
		...
		Response result = cassandraOperation.getRecordsByProperties(orgDBInfo.getKeySpace(), orgDBInfo.getTableName(),
				requestDbMap);
		List<Map<String, Object>> list = (List<Map<String, Object>>) result.get(JsonKey.RESPONSE);

		if (list.isEmpty()) {
			ProjectCommonException exception = new ProjectCommonException(ResponseCode.invalidOrgData.getErrorCode(),
	...
	private boolean validateUsrRequest(Map<String, Object> req) {
		...
		if (isNull(data.get(JsonKey.USER_ID)) && isNull(data.get(JsonKey.USERNAME))) {
			ProjectCommonException exception = new ProjectCommonException(
					ResponseCode.usrValidationError.getErrorCode(), ResponseCode.usrValidationError.getErrorMessage(),
					ResponseCode.CLIENT_ERROR.getResponseCode());
			sender().tell(exception, self());
			return false;
		}
		...
		List<Map<String, Object>> list = (List<Map<String, Object>>) result.get(JsonKey.RESPONSE);
		if (list.isEmpty()) {
			ProjectCommonException exception = new ProjectCommonException(ResponseCode.invalidUsrData.getErrorCode(),
		...
		if (!req.containsKey(JsonKey.IS_ROOT_ORG) || !(Boolean) req.get(JsonKey.IS_ROOT_ORG)) {
			...
			if (!StringUtils.isBlank(rootOrgId)) {
				...
				throw new ProjectCommonException(ResponseCode.invalidChannel.getErrorCode(),
						ResponseCode.invalidChannel.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
			}
