...
@ActorConfig(tasks = { "getRecommendedCourses" }, asyncTasks = {})
public class RecommendorActor extends BaseActor {
	...
	/**
	 * method to get the recommended contents from EkStep on basis of user
	 * language,grade and subbject .
	 * 
	 * @param request
	 * @return
	 */
	private void getRecommendedCourses(Request request) {
		...
		if (null == map) {
			...
			ProjectCommonException exception = new ProjectCommonException(
					...
					ResponseCode.invalidUserCredentials.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
		...
		try {
			...
			response.put(JsonKey.RESPONSE,
					shuffleList(Arrays.stream((Object[]) query.get(JsonKey.CONTENTS)).collect(Collectors.toList())));
	...
	/**
	 * This method will provide user details map based on passed userid, if user
	 * found then Map <Stirng,Object> will be return otherwise null.
	 * 
	 * @param userId
	 *            String
	...
	private Map<String, Object> getUserInfo(String userId) {
		...
		Response response = cassandraOperation.getRecordById(userdbInfo.getKeySpace(), userdbInfo.getTableName(),
				userId);
