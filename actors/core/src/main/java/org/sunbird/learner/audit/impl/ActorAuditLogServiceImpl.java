...
public class ActorAuditLogServiceImpl extends BaseActor implements AuditLogService {
	...
	@SuppressWarnings("unchecked")
	protected Map<String, Object> createAuditLogReqMap(AuditOperation op, Request message, Response result) {
		...
		if (message.getOperation().equals(ActorOperations.CREATE_USER.getValue())) {
			...
			if (!ProjectUtil
					.isStringNullOREmpty((String) ((Map<String, Object>) message.getRequest().get(JsonKey.ORGANISATION))
							.get(JsonKey.ORGANISATION_ID))) {
				map.put(JsonKey.OBJECT_ID, ((Map<String, Object>) message.getRequest().get(JsonKey.ORGANISATION))
						.get(JsonKey.ORGANISATION_ID));
			} else {
				map.put(JsonKey.OBJECT_ID,
						((Map<String, Object>) message.getRequest().get(JsonKey.ORGANISATION)).get(JsonKey.ID));
			}
	...
	public void save(Map<String, Object> requestedData) {
		ElasticSearchUtil.createData(EsIndex.sunbirdDataAudit.getIndexName(), EsType.history.getTypeName(),
				ProjectUtil.getUniqueIdFromTimestamp(1), requestedData);
	}
	...
	private void searchAuditHistory(Request actorMessage) {
		...
		if (ProjectUtil.isStringNullOREmpty(fromDate) && ProjectUtil.isStringNullOREmpty(toDate)) {
			...
			if (monthsDiff > 3) {
				throw new ProjectCommonException(ResponseCode.invalidDateRange.getErrorCode(),
						ResponseCode.invalidDateRange.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
			} else {
				map = new HashMap<>();
		...
		Map<String, Object> result = ElasticSearchUtil.complexSearch(searchDTO, EsIndex.sunbirdDataAudit.getIndexName(),
				EsType.history.getTypeName());
		// Decrypt user data
		...
		for (Map<String, Object> dataMap : mapList) {
			...
			if (((String) dataMap.get(JsonKey.OBJECT_TYPE)).equalsIgnoreCase(JsonKey.USER)) {
				...
				if ((null != (List<Map<String, Object>>) relationList.get(JsonKey.ADDRESS))
						&& (!((List<Map<String, Object>>) relationList.get(JsonKey.ADDRESS)).isEmpty())) {
