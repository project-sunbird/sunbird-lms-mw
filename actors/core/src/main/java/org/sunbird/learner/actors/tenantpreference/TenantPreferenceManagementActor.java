...
public class TenantPreferenceManagementActor extends BaseActor {
    ...
    private void updateTenantPreference(Request actorMessage) {
        ...
        for (Map<String, Object> map : reqList) {
            ...
            if (StringUtils.isNotBlank(key) && StringUtils.isNotBlank(data)) {
                ...
                if (!found)
                    responseList
                            .add(getResponseMap(orgId, key, "Preference setting not found for key: "
                                    + key + " for the org: " + orgId));
        ...
        for (Map<String, Object> map : req) {
            ...
            for (Map<String, Object> m : preferencesList) {
                if (key.equalsIgnoreCase((String) m.get(JsonKey.KEY))) {
                    responseList.add(getResponseMap(orgId, key,
                            "Preference already exists for key: " + key));
    ...
    private List<Map<String, Object>> getPreferencesFromDB(String orgId) {
        Response tenantPreferences =
                cassandraOperation.getRecordsByProperty(tenantPreferenceDbInfo.getKeySpace(),
    ...
    private void updateTcStatusOfUser(Request actorMessage) {
        ...
        Response response1 = cassandraOperation.getRecordById(userDbInfo.getKeySpace(),
                userDbInfo.getTableName(), requestedBy);
        ...
        ElasticSearchUtil.updateData(ProjectUtil.EsIndex.sunbird.getIndexName(),
                EsType.user.getTypeName(), requestedBy, dbMap);
        Response finalResponse = new Response();
