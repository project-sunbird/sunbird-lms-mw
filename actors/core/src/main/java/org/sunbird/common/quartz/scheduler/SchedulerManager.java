...
/**
 * 
 * This class will manage all the Quartz scheduler. We need to call the schedule
 * method at one time. we are calling this method from Util.java class.
 * 
 * @author Manzarul
 *
 */
public final class SchedulerManager {
	...
	public static void scheduleChannelReg(String identifier) {
		...
		// interface.
		JobDetail channelRegistrationJob = JobBuilder.newJob(ChannelRegistrationScheduler.class).requestRecovery(true)
				.withIdentity("channelRegistrationScheduler", identifier).build();
	...
	private void scheduleUpdateUserCountJob(String identifier) {
		...
		// 1- create a job and bind with class which is implementing Job
		// interface.
		JobDetail updateUserCountJob = JobBuilder.newJob(UpdateUserCountScheduler.class).requestRecovery(true)
				.withIdentity("updateUserCountScheduler", identifier).build();

		// 2- Create a trigger object that will define frequency of run.
		// This will run every day 2:00 AM
		Trigger updateUserCountTrigger = TriggerBuilder.newTrigger().withIdentity("updateUserCountTrigger", identifier)
	...
	private void scheduleMetricsJob(String identifier) {
		...
		Trigger metricsTrigger = TriggerBuilder.newTrigger().withIdentity("metricsTrigger", identifier).withSchedule(
				CronScheduleBuilder.cronSchedule(PropertiesCache.getInstance().getProperty("quartz_metrics_timer")))
				.build();
		try {
	...
	private void scheduleMetricsReportJob(String identifier) {
		...
		Trigger metricsReportRetryTrigger = TriggerBuilder.newTrigger()
				.withIdentity("metricsReportRetryTrigger", identifier).withSchedule(CronScheduleBuilder
						.cronSchedule(PropertiesCache.getInstance().getProperty("quartz_matrix_report_timer")))
				.build();
		try {
	...
	private void scheduleCoursePublishJob(String identifier) {
		...
		// 1- create a job and bind with class which is implementing Job
		// interface.
		JobDetail coursePublishedJob = JobBuilder.newJob(CoursePublishedUpdate.class).requestRecovery(true)
				.withIdentity("coursePublishedScheduler", identifier).build();
		...
		Trigger coursePublishedTrigger = TriggerBuilder.newTrigger().withIdentity("coursePublishedTrigger", identifier)
				.withSchedule(CronScheduleBuilder
						.cronSchedule(PropertiesCache.getInstance().getProperty("quartz_course_publish_timer")))
				.build();
		try {
	...
	private void scheduleBulkUploadJob(String identifier) {
		...
		// 1- create a job and bind with class which is implementing Job
		// interface.
		JobDetail uploadVerifyJob = JobBuilder.newJob(UploadLookUpScheduler.class).requestRecovery(true)
				.withIdentity("uploadVerifyScheduler", identifier).build();
