	...
	static {
		try {
			...
			if ("remote".equalsIgnoreCase(cache.getProperty("background_actor_provider"))) {
				ProjectLogger.log(
						"Initializing Remote Actor System in org.sunbird.learner.util.actorutility.ActorSystemFactory");
				createRemoteActorSystem();
			} else {
				ProjectLogger.log(
						"Initializing Local Actor System in org.sunbird.learner.util.actorutility.ActorSystemFactory");
