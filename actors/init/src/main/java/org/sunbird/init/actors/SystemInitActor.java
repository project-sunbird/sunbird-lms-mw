...
public class SystemInitActor extends BaseActor {
  ...
  private void systemInitRootOrg(Request actorMessage) {
    ...
    if (isNull(initStatus) || isNull(initStatus.getValue())) {
      ...
      ProjectLogger.log(
          "SystemInitActor: systemInitRootOrg - error in rootOrg Init status "
              + initStatus.getValue(),
          LoggerEnum.DEBUG.name());
      ProjectCommonException.throwServerErrorException(
          ResponseCode.internalError, ResponseCode.internalError.getErrorMessage());
    }
  }
  ...
  private Map<String, Object> getRootOrgIfExists() {
    ...
    List<Map<String, Object>> orgList = (List<Map<String, Object>>) response.get(JsonKey.RESPONSE);
    if (orgList.size() > 0) {
   ...
   */
  private boolean getRootOrgInfoFromIndex() {
   ...
   */
  private void saveRootOrgInfoToIndex(Map<String, Object> req) {
  ...
  private void checkAndCreateRootOrg(Request actorMessage) {
    ...
    String uniqueId = (String) req.get(JsonKey.ID);
    if (isNull(rootOrgDetails)) {
      ...
      saveRootOrgInfoToIndex(req);
    } else {
      result.getResult().put(JsonKey.ORGANISATION_ID, rootOrgDetails.get(JsonKey.ID));
      sender().tell(result, self());
      if (!getRootOrgInfoFromIndex()) {
        saveRootOrgInfoToIndex(rootOrgDetails);
      }
    }
  }
  ...
  private void setRootOrgInitialisationStatus(String status) {
    ProjectLogger.log(
        "SystemInitActor: setRootOrgInitialisationStatus called", LoggerEnum.DEBUG.name());
    for (int i = 0; i <= WRITE_SETTINGS_RETRIES_ALLOWED; i++) {
      try {
        ...
        if (i == WRITE_SETTINGS_RETRIES_ALLOWED) {
          ProjectLogger.log(
              "Max retries reached while updating rootOrg init status.", LoggerEnum.DEBUG.name());
          ProjectCommonException.throwServerErrorException(
  ...
  private SystemSetting getRootOrgInitialisationStatus() {
    ...
    ProjectLogger.log(
        "SystemInitActor: getRootOrgInitialisationStatus called", LoggerEnum.DEBUG.name());
    try {
