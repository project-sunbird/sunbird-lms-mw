...
public class BadgeClassActor extends BaseActor {
    ...
    private void createBadgeClass(Request actorMessage) {
        ...
        try {
            ...
            String badgrResponseStr = (String) HttpUtil.postFormData(formParams, fileParams, headers, BadgingUtil.getBadgeClassUrl(issuerId)).get(JsonKey.BODY);
    ...
    private void listBadgeClass(Request actorMessage) {
        ...
        try {
            ...
            List<BadgeClassExtension> badgeClassExtList = badgeClassExtensionService.get(issuerList, rootOrgId, type, subtype, allowedRoles);
    ...
    private List<Object> listBadgeClassForIssuer(String issuerSlug, List<BadgeClassExtension> badgeClassExtensionList, List<String> allowedRoles) throws IOException {
        Map<String, String> headers = BadgingUtil.getBadgrHeaders();
        ...
        for (Map<String, Object> badge : badges) {
            BadgeClassExtension matchedBadgeClassExt = badgeClassExtensionList.stream().filter(x -> x.getBadgeId().equals(badge.get(BadgingJsonKey.SLUG))).findFirst().orElse(null);
    ...
    private void deleteBadgeClass(Request actorMessage) {
        ...
        try {
            ...
            String badgrResponseStr = (String) HttpUtil.sendDeleteRequest(headers, badgrUrl).get(JsonKey.BODY);
