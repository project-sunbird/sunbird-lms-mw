...
public class OrganisationMetricsActor extends BaseMetricsActor {
	...
	private String getQueryRequest(String periodStr, String orgId, String operation) {
		...
		builder.append("{\"request\":{\"rawQuery\":{\"query\":{\"filtered\":")
				.append("{\"query\":{\"bool\":{\"must\":[{\"query\":{\"range\":{\"").append(operationMap.get("dateKey"))
				.append("\":{\"gte\":\"")
				...
				.append(",{\"match\":{\"status.raw\":\"" + operationMap.get("status")).append("\"}}]}}}},\"aggs\":{\"");
		...
		builder.append("status\":{\"terms\":{\"field\":\"status.raw\",\"include\":[\"")
				...
				.append("\",\"time_zone\":\"+05:30\",\"extended_bounds\":{\"min\":")
				.append(dateMap.get(STARTTIMEMILIS) + ",\"max\":").append(dateMap.get(ENDTIMEMILIS)).append("}}}}}");
	...
	private static Map<String, Object> putAggregationMap(String responseStr, Map<String, Object> aggregationMap,
			String operation) {
		try {
	...
	private void orgCreationMetrics(Request actorMessage) {
		...
		try {
			...
			Map<String, Object> aggregationMap = (Map<String, Object>) cache.getData(JsonKey.OrgCreation, orgId,
					periodStr);
			...
			Response response = metricsResponseGenerator(responseFormat, periodStr, getViewData(orgId, orgName));
			sender().tell(response, self());
		...
		try {
			...
			if (null == rootOrgData || rootOrgData.isEmpty()) {
				ProjectCommonException exception = new ProjectCommonException(
						...
						ResponseCode.invalidRootOrgData.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
				sender().tell(exception, self());
				return;
			}
	...
	private String getOrgConsumptionData(Request actorMessage, String periodStr, String orgHashId, String channel)
			throws IOException {
	...
	private String getOrgMetricsRequest(Request actorMessage, String periodStr, String orgHashId, String userId,
			String channel) throws JsonProcessingException {
	...
	private Map<String, Object> validateOrg(String orgId) {
		try {
			Map<String, Object> result = ElasticSearchUtil.getDataByIdentifier(
					ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.organisation.getTypeName(), orgId);
