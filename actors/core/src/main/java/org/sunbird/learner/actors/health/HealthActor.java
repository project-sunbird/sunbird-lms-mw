...
public class HealthActor extends BaseActor {
	...
	private void saveUserBadges(Request actorMessage) {
		...
		if (map == null || map.size() == 0) {
			ProjectCommonException ex = new ProjectCommonException(ResponseCode.invalidReceiverId.getErrorCode(),
					ResponseCode.invalidReceiverId.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
			sender().tell(ex, self());
			return;
		}

		Response response = cassandraOperation.getRecordById(badgesDbInfo.getKeySpace(), badgesDbInfo.getTableName(),
				badgeTypeId);
		if (response != null && response.get(JsonKey.RESPONSE) != null) {
			...
			if (badgesListMap == null || badgesListMap.isEmpty()) {
				ProjectCommonException ex = new ProjectCommonException(ResponseCode.invalidBadgeTypeId.getErrorCode(),
						ResponseCode.invalidBadgeTypeId.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
				sender().tell(ex, self());
				return;
			}
		} else {
	...
	private void getBadges() {
		try {
			Response response = cassandraOperation.getAllRecords(badgesDbInfo.getKeySpace(),
					badgesDbInfo.getTableName());
			sender().tell(response, self());
		} catch (Exception e) {
