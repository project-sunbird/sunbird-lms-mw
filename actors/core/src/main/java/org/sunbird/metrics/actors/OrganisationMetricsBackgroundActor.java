...
@RouterConfig(request = {}, bgRequest = { "orgCreationMetricsData", "orgConsumptionMetricsData" })
public class OrganisationMetricsBackgroundActor extends BaseMetricsActor {
	...
	public void onReceive(Request request) throws Throwable {
		if (request.getOperation().equalsIgnoreCase(ActorOperations.ORG_CREATION_METRICS_DATA.getValue())) {
			orgCreationMetricsData(request);
		} else if (request.getOperation().equalsIgnoreCase(ActorOperations.ORG_CONSUMPTION_METRICS_DATA.getValue())) {
	...
	private void orgConsumptionMetricsData(Request actorMessage) {
		...
		try {
			...
			String channel = (String) (orgData.get(JsonKey.CHANNEL) == null ? "" : orgData.get(JsonKey.CHANNEL));
			...
			for (Map<String, Object> userData : usersData) {
				...
				Map<String, Object> ekstepData = getConsumptionDataFromResponse(esResponse, userData,
						(List<String>) (Object) headers);
	...
	private List<Map<String, Object>> getUserDetailsFromES(List<Map<String, Object>> ekstepData) {
		...
		for (Map<String, Object> userData : ekstepData) {
			...
			try {
				Map<String, Object> result = ElasticSearchUtil.complexSearch(
						createESRequest(filter, null, coursefields), ProjectUtil.EsIndex.sunbird.getIndexName(),
						EsType.user.getTypeName());
				if (null != result && !result.isEmpty()) {
					List<Map<String, Object>> resultList = (List<Map<String, Object>>) result.get(JsonKey.CONTENT);
	...
	private List<Map<String, Object>> getDataFromResponse(String responseData, List<Object> headers, String orgId) {
		...
		try {
			...
			List<Map<String, Object>> resultList = (List<Map<String, Object>>) resultData.get(JsonKey.CONTENT);
	...
	private static Map<String, String> conceptList() {
		...
		for (String domain : domains) {
			...
			try {
				...
				List<Map<String, Object>> conceptData = (List<Map<String, Object>>) responseMap.get("concepts");
	...
	@SuppressWarnings("unchecked")
	private Map<String, Object> getConsumptionDataFromResponse(String responseData, Map<String, Object> userData,
			List<String> headers) {
	...
	private String getJsonString(Object requestObject) {
		...
		try {
			...
			throw new ProjectCommonException(ResponseCode.invalidJsonData.getErrorCode(),
					ResponseCode.invalidJsonData.getErrorMessage(), ResponseCode.SERVER_ERROR.getResponseCode());
		}
		return data;
	}
