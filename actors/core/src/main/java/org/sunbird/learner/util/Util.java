...
public final class Util {
	...
	private static void initializeOrgStatusTransition() {
		...
		orgStatusTransition.put(OrgStatus.BLOCKED.getValue(),
				Arrays.asList(OrgStatus.ACTIVE.getValue(), OrgStatus.BLOCKED.getValue(), OrgStatus.RETIRED.getValue()));
	...
	public static boolean registerChannel(Map<String, Object> req) {
		...
		String regStatus = "";
		try {
			ProjectLogger.log("start call for registering the channel for hashTag id ==" + req.get(JsonKey.HASHTAGID));
			...
			regStatus = HttpUtil.sendPostRequest(
					(ekStepBaseUrl + PropertiesCache.getInstance().getProperty(JsonKey.EKSTEP_CHANNEL_REG_API_URL)),
					reqString, headerMap);
			ProjectLogger.log("end call for channel registration for hashTag id ==" + req.get(JsonKey.HASHTAGID));
		} catch (Exception e) {
	...
	public static boolean updateChannel(Map<String, Object> req) {
		...
		try {
			...
			regStatus = HttpUtil.sendPatchRequest(
					(ekStepBaseUrl + PropertiesCache.getInstance().getProperty(JsonKey.EKSTEP_CHANNEL_UPDATE_API_URL))
	...
	public static void initializeContextForSchedulerJob(String actorType, String actorId, String environment) {
	...
	private static Map<String, Object> getOrgDetails(String identifier) {
		...
		Response response = cassandraOperation.getRecordById(orgDbInfo.getKeySpace(), orgDbInfo.getTableName(),
				identifier);
