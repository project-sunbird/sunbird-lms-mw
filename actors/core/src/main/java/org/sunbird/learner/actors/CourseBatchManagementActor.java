...
public class CourseBatchManagementActor extends BaseActor {
	...
	private void getCourseBatchDetail(Request actorMessage) {
		...
		Response result = cassandraOperation.getRecordById(dbInfo.getKeySpace(), dbInfo.getTableName(), batchId);
	...
	private void addUserCourseBatch(Request actorMessage) {
		...
		for (String userId : userIds) {
			if (!(participants.containsKey(userId))) {
				...
				participants.put(userId, addUserCourses(batchId, (String) courseBatchObject.get(JsonKey.COURSE_ID),
	...
	private void createCourseBatch(Request actorMessage) {
		...
		if (req.containsKey(JsonKey.COURSE_CREATED_FOR) && req.get(JsonKey.COURSE_CREATED_FOR) instanceof List) {
		...
		ProjectLogger.log(
				"Course Batch data saving to ES Completed -- for Id " + uniqueId + " with response ==" + esResponse,
	...
	private Map<String, String> getAdditionalCourseInfo(Map<String, Object> ekStepContent) {
		...
		courseMap.put(JsonKey.COURSE_LOGO_URL,
				(String) ekStepContent.getOrDefault(JsonKey.APP_ICON, "") != null
						...
						: "");
		courseMap.put(JsonKey.COURSE_NAME,
		...
		if (ProjectUtil.isNotNull(ekStepContent.get(JsonKey.LEAF_NODE_COUNT))) {
			courseMap.put(JsonKey.LEAF_NODE_COUNT, ((Integer) ekStepContent.get(JsonKey.LEAF_NODE_COUNT)).toString());
		}
	...
	private void updateCourseBatch(Request actorMessage) {
		...
		req.remove(JsonKey.PARTICIPANT);
		if (!ProjectUtil.isStringNullOREmpty(((String) req.get(JsonKey.HASHTAGID)))) {
			req.put(JsonKey.HASHTAGID, validateHashTagId(((String) req.get(JsonKey.HASHTAGID)), JsonKey.UPDATE,
					(String) req.get(JsonKey.ID)));
		}
		...
		if (null != resList && !resList.isEmpty()) {
			...
			if (null != endDate && endDate.before(todate)) {
				throw new ProjectCommonException(ResponseCode.BatchCloseError.getErrorCode(),
						ResponseCode.BatchCloseError.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
			}
			...
			Response result = cassandraOperation.updateRecord(dbInfo.getKeySpace(), dbInfo.getTableName(), req);
			sender().tell(result, self());

			targetObject = TelemetryUtil.generateTargetObject((String) req.get(JsonKey.ID), JsonKey.BATCH,
					JsonKey.UPDATE, null);
			TelemetryUtil.telemetryProcessingCall(req, targetObject, correlatedObject);
