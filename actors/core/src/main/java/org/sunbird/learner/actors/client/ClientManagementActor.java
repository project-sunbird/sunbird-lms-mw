...
@RouterConfig(request = { "registerClient", "updateClientKey", "getClientKey" }, bgRequest = {})
public class ClientManagementActor extends BaseActor {
	...
	private void registerClient(Request actorMessage) {
		...
		if (!dataList.isEmpty() && dataList.get(0).containsKey(JsonKey.ID)) {
			throw new ProjectCommonException(ResponseCode.invalidClientName.getErrorCode(),
					ResponseCode.invalidClientName.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
		}
		...
		Response result = cassandraOperation.insertRecord(clientDbInfo.getKeySpace(), clientDbInfo.getTableName(), req);
	...
	private void updateClientKey(Request actorMessage) {
		...
		List<Map<String, Object>> dataList = (List<Map<String, Object>>) data.getResult().get(JsonKey.RESPONSE);
		if (dataList.isEmpty()
				|| !StringUtils.equalsIgnoreCase(masterKey, (String) dataList.get(0).get(JsonKey.MASTER_KEY))) {
		...
		Response result = cassandraOperation.updateRecord(clientDbInfo.getKeySpace(), clientDbInfo.getTableName(), req);
	...
	private Response getDataFromCassandra(String propertyName, String propertyValue) {
		...
		if (StringUtils.equalsIgnoreCase(JsonKey.CLIENT_NAME, propertyName)) {
			result = cassandraOperation.getRecordsByProperty(clientDbInfo.getKeySpace(), clientDbInfo.getTableName(),
					JsonKey.CLIENT_NAME, propertyValue.toLowerCase());
		} else if (StringUtils.equalsIgnoreCase(JsonKey.ID, propertyName)) {
			result = cassandraOperation.getRecordsByProperty(clientDbInfo.getKeySpace(), clientDbInfo.getTableName(),
					JsonKey.ID, propertyValue);
		} else if (StringUtils.equalsIgnoreCase(JsonKey.CHANNEL, propertyName)) {
			result = cassandraOperation.getRecordsByProperty(clientDbInfo.getKeySpace(), clientDbInfo.getTableName(),
					JsonKey.CHANNEL, propertyValue);
		}
