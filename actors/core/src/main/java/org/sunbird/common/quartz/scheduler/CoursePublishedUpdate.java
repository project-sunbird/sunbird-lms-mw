...
/**
 * This class will call the EKstep get content api to know the status of course
 * published. once course status becomes live then it will update status under
 * course published table and collect all participant from course-batch table
 * and register all those participant under user_course table and push the data
 * to ES.
 * 
 * @author Manzarul
 *
...
public class CoursePublishedUpdate implements Job {
			...
			if (null != ekStepResult && !ekStepResult.isEmpty()) {
				...
				for (String courseId : ekStepResult) {
					try {
						...
						Response response = cassandraOperation.getRecordsByProperty(courseBatchDBInfo.getKeySpace(),
								courseBatchDBInfo.getTableName(), JsonKey.COURSE_ID, courseId);
						List<Map<String, Object>> batchList = (List<Map<String, Object>>) response
					...
					if (!entry.getValue()) {
						...
						try {
							...
							TelemetryUtil.generateCorrelatedObject(entry.getKey(), JsonKey.USER, null,
									correlatedObject);
							TelemetryUtil.generateCorrelatedObject((String) batch.get(JsonKey.ID), JsonKey.BATCH, null,
									correlatedObject);
	...
	private void updateCourseStatusTable(List<String> ekStepResult) {
		...
		for (String courseId : ekStepResult) {
			...
			try {
				cassandraOperation.updateRecord(coursePublishDBInfo.getKeySpace(), coursePublishDBInfo.getTableName(),
						map);
	...
	/**
	 * This method will provide list of all those course ids , which we did the
	 * published but haven't got the status back as live.
	 * 
	 * @return List<String>
	 */
	@SuppressWarnings("unchecked")
	private List<String> getAllUnPublishedCourseStatusId() {
		...
		Response response = cassandraOperation.getRecordsByProperty(coursePublishDBInfo.getKeySpace(),
				coursePublishDBInfo.getTableName(), JsonKey.STATUS, ProjectUtil.CourseMgmtStatus.DRAFT.ordinal());
