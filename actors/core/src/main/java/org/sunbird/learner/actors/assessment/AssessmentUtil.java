...
public class AssessmentUtil {
	...
	/**
	 * This method will do the user assessment evaluation.once evaluation is done
	 * data will be stored into different-2 tables with there evaluated result.
	 * 
	 * @param request
	 *            Map<String, Object>
	 */
	public void evalAssessment(Map<String, Object> request) {
		...
		for (Map<String, Object> map : list) {
			...
			try {
				cassandraOperation.upsertRecord(assmntItemDbInfo.getKeySpace(), assmntItemDbInfo.getTableName(),
						reqMap);
			} catch (Exception e) {
	...
	/**
	 * This method will fetch user all unevaluated assessment data. request map will
	 * contain courseId, contentId, userId values only.
	 * 
	 * @param request
	 *            Map<String, Object>
	...
	private Map<String, List<Map<String, Object>>> createReqFrEvalAssmnt(Map<String, Object> request) {
		...
		/**
		 * fetching records from Assessment item table based on courseId,contentId and
		 * userId and creating request for evaluateAssessment method of
		 * DefaultAssessmentEvaluator class
		 */
		...
		Response response = cassandraOperation.getRecordsByProperties(assmntItemDbInfo.getKeySpace(),
				assmntItemDbInfo.getTableName(), request);
	...
	/**
	 * This method will update user assessment score under content consumption
	 * table.
	 * 
	 * @param assmntListResult
	...
	private void updateScoreIntoContentConsumptionTable(List<Map<String, Object>> assmntListResult) {
		...
		for (Map<String, Object> map : assmntListResult) {
			...
			String id = OneWayHashing.encryptVal(
					userId + JsonKey.PRIMARY_KEY_DELIMETER + contentId + JsonKey.PRIMARY_KEY_DELIMETER + courseId);
	...
	private void insertRecordIntoAssmntEvalTable(List<Map<String, Object>> assmntList, Map<String, Object> assmntReq) {
		for (Map<String, Object> map : assmntList) {
			try {
				...
				Response response = cassandraOperation.getRecordsByProperties(assmntEvalDbInfo.getKeySpace(),
						assmntEvalDbInfo.getTableName(), request);
				...
				if (null != resList && resList.size() > 0) {
					...
					Double scoreFrmDbRes = (resMap.get(JsonKey.ASSESSMENT_SCORE)) != null
							? Double.parseDouble((String) resMap.get(JsonKey.ASSESSMENT_SCORE))
							: 0d;
					...
					if (scoreFrmDbRes > scoreFrmReq) {
						...
						int attemptedCount = (resMap.get(JsonKey.ATTEMPTED_COUNT) != null)
								? (int) resMap.get(JsonKey.ATTEMPTED_COUNT)
								: 0;
						reqMap.put(JsonKey.ATTEMPTED_COUNT, attemptedCount + 1);
						reqMap.put(JsonKey.ID, resMap.get(JsonKey.ID));
						cassandraOperation.updateRecord(assmntEvalDbInfo.getKeySpace(), assmntEvalDbInfo.getTableName(),
								map);
					} else {
					...
					cassandraOperation.insertRecord(assmntEvalDbInfo.getKeySpace(), assmntEvalDbInfo.getTableName(),
							map);
				}
			} catch (Exception e) {
				ProjectLogger.log(e.getMessage(), e);
