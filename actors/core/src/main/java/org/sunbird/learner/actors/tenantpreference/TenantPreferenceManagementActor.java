...
public class TenantPreferenceManagementActor extends BaseActor {
	...
	private void getTenantPreference(Request actorMessage) {
		...
		Map<String, Map<String, Object>> defaultPrefMap = getPreferenceMap(getPreferencesFromDB(DEFAULT_WILDCARD_ORG_ID));
		
		Response finalResponse = new Response();
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		if (null != keys && !keys.isEmpty()) {
			for (String key : keys) {
				if (null != orgPrefMap.get(key))
					list.add(orgPrefMap.get(key));
				else if (null != defaultPrefMap.get(key))
					list.add(defaultPrefMap.get(key));
			}
		} else {
			if (!orgPrefMap.isEmpty())
				list.addAll(orgPrefMap.values());
			else if (!defaultPrefMap.isEmpty())
				list.addAll(defaultPrefMap.values());
		}
		...
		for (Map<String, Object> map : reqList) {
			...
			if (StringUtils.isNotBlank(key) && StringUtils.isNotBlank(data)) {
				...
				for (Map<String, Object> m : preferencesList) {
					...
					if (key.equals((String) m.get(JsonKey.KEY))) {
						preferenceObj = m;
						found = true;
						break;
					}
				...
				if (!found) 
					responseList.add(getResponseMap(orgId, key, "Preference setting not found for key: " + key + " for the org: " + orgId));
				...
				if (null != preferenceObj) {
					...
					cassandraOperation.updateRecord(tenantPreferenceDbInfo.getKeySpace(),
							tenantPreferenceDbInfo.getTableName(), preferenceObj);
	...
	/**
	 * Method to create tenant preference for an org , if already
	 * exists it will not create new one.
	 * 
	 * @param actorMessage
	 */
	@SuppressWarnings("unchecked")
	...
	private Map<String, Object> getResponseMap(String orgId, String key, String error) {
		...
		if (StringUtils.isNotBlank(error)) {
			...
			responseMap.put(JsonKey.ERROR, "Preference setting not found for key: " + key + " for the org: " + orgId);
		} else
	...
	private List<Map<String, Object>> getPreferencesFromDB(String orgId) {
		Response tenantPreferences = cassandraOperation.getRecordsByProperty(tenantPreferenceDbInfo.getKeySpace(),
				tenantPreferenceDbInfo.getTableName(), JsonKey.ORG_ID, orgId);
		List<Map<String, Object>> preferencesList = (List<Map<String, Object>>) tenantPreferences.get(JsonKey.RESPONSE);
	...
	private void validateRequest(Request actorMessage) {
		...
		/*
		// check if the org id is either a valid org in the system or is the default wild card org (*)
		if (!StringUtils.equalsIgnoreCase(DEFAULT_WILDCARD_ORG_ID, orgId)) {
