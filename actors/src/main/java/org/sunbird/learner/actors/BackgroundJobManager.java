...
public class BackgroundJobManager extends UntypedAbstractActor {
	...
	private void addBadgeToUserprofile(Request actorMessage) {
		...
		Response response = cassandraOperation.getRecordsByProperties(userbadge.getKeySpace(), userbadge.getTableName(),
				userBadgeMap);
		if (response != null && response.get(JsonKey.RESPONSE) != null) {
			...
			if (badgesList != null && !badgesList.isEmpty()) {
				...
				boolean updateResponse = updateDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(),
						ProjectUtil.EsType.user.getTypeName(), (String) userBadgeMap.get(JsonKey.RECEIVER_ID), map);
	...
	private void updateUserRoleToEs(Request actorMessage) {
		...
		updateDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.user.getTypeName(),
	...
	private void updateUserCourseInfoToEs(Request actorMessage) {
		...
		updateDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.usercourses.getTypeName(),
	...
	private void insertUserCourseInfoToEs(Request actorMessage) {
		...
		Map<String, Object> batch = (Map<String, Object>) actorMessage.getRequest().get(JsonKey.USER_COURSES);
		insertDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.usercourses.getTypeName(),
				(String) batch.get(JsonKey.ID), batch);
	...
	private void updateCourseBatchInfoToEs(Request actorMessage) {
		...
		updateDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.course.getTypeName(),
				(String) batch.get(JsonKey.ID), batch);
	}
	...
	private void insertCourseBatchInfoToEs(Request actorMessage) {
		...
		registertag((String) batch.getOrDefault(JsonKey.COURSE_ID, (String) batch.get(JsonKey.COURSE_ID)), "{}",
				CourseBatchSchedulerUtil.headerMap);
	}
	...
	private void insertOrgInfoToEs(Request actorMessage) {
		...
		if (ProjectUtil.isNotNull(orgMap)) {
			...
			Response orgResponse = cassandraOperation.getRecordById(orgDbInfo.getKeySpace(), orgDbInfo.getTableName(),
					id);
			List<Map<String, Object>> orgList = (List<Map<String, Object>>) orgResponse.getResult()
	...
	private void updateUserCount(Request actorMessage) {
		...
		Response result = cassandraOperation.getPropertiesValueById(dbInfo.getKeySpace(), dbInfo.getTableName(),
				courseId, JsonKey.USER_COUNT);
	...
	private String getCourseData(String contnetId) {
		...
		try {
			...
			responseData = HttpUtil.sendGetRequest(
					ekStepBaseUrl + PropertiesCache.getInstance().getProperty(JsonKey.EKSTEP_CONTENT_URL) + contnetId,
					headerMap);
		} catch (IOException e) {
			ProjectLogger.log(e.getMessage(), e);
		}
	...
	private Map<String, Object> getContentDetails(String content) {
		...
		try {
			...
			HashMap<String, Map<String, Object>> result = mapper.readValue(resultObj.toString(), HashMap.class);
	...
	private void insertUserNotesToEs(Request actorMessage) {
		...
		if (ProjectUtil.isNotNull(noteMap) && noteMap.size() > 0) {
			...
			insertDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.usernotes.getTypeName(),
					id, noteMap);
		} else {
	...
	private void updateUserNotesToEs(Request actorMessage) {
		Map<String, Object> noteMap = (Map<String, Object>) actorMessage.getRequest().get(JsonKey.NOTE);
		updateDataToElastic(ProjectUtil.EsIndex.sunbird.getIndexName(), ProjectUtil.EsType.usernotes.getTypeName(),
