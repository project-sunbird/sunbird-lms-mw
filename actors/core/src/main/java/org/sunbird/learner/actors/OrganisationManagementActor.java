...
public class OrganisationManagementActor extends UntypedAbstractActor {
        ...
        if (message instanceof Request) {
            try {
                ...
                if (actorMessage.getOperation()
                        .equalsIgnoreCase(ActorOperations.CREATE_ORG.getValue())) {
    ...
    private void updateOrgType(Request actorMessage) {
        ...
        try {
            ...
            targetObject = TelemetryUtil.generateTargetObject((String) request.get(JsonKey.ID),
                    "organisationType", JsonKey.UPDATE, null);
            ...
            new Thread() {
                @Override
                public void run() {
                    if (((String) response.get(JsonKey.RESPONSE))
    ...
    private void createOrgType(Request actorMessage) {
        ...
        try {
            ...
            targetObject = TelemetryUtil.generateTargetObject(id, "organisationType",
                    JsonKey.CREATE, null);
            TelemetryUtil.telemetryProcessingCall(actorMessage.getRequest(), targetObject,
                    correlatedObject);
    ...
    private void createOrg(Request actorMessage) {
        ...
        try {
            ...
            if (req.containsKey(JsonKey.PROVIDER) || req.containsKey(JsonKey.EXTERNAL_ID)) {
                ...
                if (req.containsKey(JsonKey.CHANNEL)) {
                    ...
                    if (req.containsKey(JsonKey.PROVIDER)) {
                        String rootOrgId =
                                getRootOrgIdFromChannel((String) req.get(JsonKey.PROVIDER));
            ...
            Response result = cassandraOperation.insertRecord(orgDbInfo.getKeySpace(),
                    orgDbInfo.getTableName(), req);
    ...
    private String validateLocationId(String locId) {
        ...
        Response response = cassandraOperation.getRecordById(geoLocDbInfo.getKeySpace(),
                geoLocDbInfo.getTableName(), locId);
    ...
    private String validateOrgType(String orgType) {
        ...
        if (!ProjectUtil
                .isStringNullOREmpty(DataCacheHandler.getOrgTypeMap().get(orgType.toLowerCase()))) {
    ...
    private void updateOrgData(Request actorMessage) {
        ...
        try {
            ...
            if (updateOrgDBO.containsKey(JsonKey.CHANNEL)) {
                ...
                String slug = Slug.makeSlug((String) updateOrgDBO.getOrDefault(JsonKey.CHANNEL, ""),
                        true);
                if ((boolean) orgDBO.get(JsonKey.IS_ROOT_ORG)) {
                    String rootOrgId = getRootOrgIdFromSlug(slug);
                    if (ProjectUtil.isStringNullOREmpty(rootOrgId)
                            || (!ProjectUtil.isStringNullOREmpty(rootOrgId) && rootOrgId
                                    .equalsIgnoreCase((String) orgDBO.get(JsonKey.ID)))) {
            ...
            if (null != orgDBO.get(JsonKey.IS_ROOT_ORG)
                    && (boolean) orgDBO.get(JsonKey.IS_ROOT_ORG)) {
            ...
            if (updateOrgDBO.containsKey(JsonKey.CONTACT_DETAILS)) {
                ...
                if (listOfMap != null && !listOfMap.isEmpty()) {
                    ObjectMapper mapper = new ObjectMapper();
                    try {
                        updateOrgDBO.put(JsonKey.CONTACT_DETAILS,
                                mapper.writeValueAsString(listOfMap));
                    } catch (IOException e) {
                        ProjectLogger.log(e.getMessage(), e);
                    }
                }
    ...
    private void addMemberOrganisation(Request actorMessage) {
        ...
        if (!list.isEmpty()) {
            ...
            if (null != tempOrgap && !((boolean) tempOrgap.get(JsonKey.IS_DELETED))) {
                ...
                response.getResult().put(JsonKey.RESPONSE,
                        ResponseMessage.Message.EXISTING_ORG_MEMBER);
                sender().tell(response, self());
                return;
        ...
        if (((String) response.get(JsonKey.RESPONSE)).equalsIgnoreCase(JsonKey.SUCCESS)) {
            ...
            try {
                ...
                ProjectLogger.log(
                        "Exception Occured during saving user to Es while addMemberOrganisation : ",
                        ex);
            }
        } else {
            ...
            if (!orgList.isEmpty()) {
                ...
                if (isNotNull(orgMap.get(JsonKey.NO_OF_MEMBERS.toLowerCase()))) {
                    Integer count = Integer
                            .valueOf((String) orgMap.get(JsonKey.NO_OF_MEMBERS.toLowerCase()));
                    newOrgMap.put(JsonKey.ID, orgId);
    ...
    /**
     * Inserts the organization to organization relation is not exists, else Updates the
     * organization to organization relation
     */
    ...
    public void validateRootOrg(Map<String, Object> request) {
        ...
        if (!ProjectUtil.isStringNullOREmpty((String) request.get(JsonKey.PARENT_ORG_ID))) {
            ...
            if (!ProjectUtil.isStringNullOREmpty((String) parentOrgDBO.get(JsonKey.ROOT_ORG_ID))) {
                ...
                if (null != request.get(JsonKey.ROOT_ORG_ID) && !parentRootOrg
                        .equalsIgnoreCase(request.get(JsonKey.ROOT_ORG).toString())) {
    ...
    public boolean validateChannelIdForRootOrg(Map<String, Object> request) {
        ...
        if (null != request.get(JsonKey.IS_ROOT_ORG)
    ...
    public void validateCyclicRelationForOrganisation(Map<String, Object> request) {
        if (!ProjectUtil.isStringNullOREmpty((String) request.get(JsonKey.PARENT_ORG_ID))
                && isChildOf((String) request.get(JsonKey.ORGANISATION_ID),
        ...
        if (!ProjectUtil.isStringNullOREmpty((String) data.get(JsonKey.USER_ID))) {
            ...
            if (data.containsKey(JsonKey.PROVIDER)
                ...
                data.put(JsonKey.LOGIN_ID, (String) data.get(JsonKey.USERNAME) + "@"
                        + (String) data.get(JsonKey.PROVIDER));
            } else {
