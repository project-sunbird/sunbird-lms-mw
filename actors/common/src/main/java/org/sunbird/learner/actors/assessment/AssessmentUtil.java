...
public class AssessmentUtil {
  ...
  /**
   * This method will do the user assessment evaluation.once evaluation is done data will be stored
   * into different-2 tables with there evaluated result.
   *
    ...
    for (Map<String, Object> map : list) {
      ...
      try {
        cassandraOperation.upsertRecord(
            assmntItemDbInfo.getKeySpace(), assmntItemDbInfo.getTableName(), reqMap);
      } catch (Exception e) {
  ...
  /**
   * This method will fetch user all unevaluated assessment data. request map will contain courseId,
   * contentId, userId values only.
   *
   * @param request Map<String, Object>
   * @return Map<String, List<Map<String, Object>>>
   */
  @SuppressWarnings("unchecked")
  private Map<String, List<Map<String, Object>>> createReqFrEvalAssmnt(
      Map<String, Object> request) {
    Map<String, List<Map<String, Object>>> result = new HashMap<>();
    List<Map<String, Object>> resMap = null;
    /**
     * fetching records from Assessment item table based on courseId,contentId and userId and
     * creating request for evaluateAssessment method of DefaultAssessmentEvaluator class
     */
    request.put(JsonKey.PROCESSING_STATUS, false);
    Response response =
        cassandraOperation.getRecordsByProperties(
            assmntItemDbInfo.getKeySpace(), assmntItemDbInfo.getTableName(), request);
      ...
      try {
        Response response =
            cassandraOperation.getRecordById(
                contentConsumptionDbInfo.getKeySpace(),
                contentConsumptionDbInfo.getTableName(),
                id);
        ...
        if (null != responseList && responseList.isEmpty()) {
          ...
          if (((Double.parseDouble((String) contentConsumptionMap.get(JsonKey.ASSESSMENT_SCORE)))
                  - (Double.parseDouble((String) reqMap.get(JsonKey.ASSESSMENT_SCORE))))
              < 0) {
            cassandraOperation.upsertRecord(
                contentConsumptionDbInfo.getKeySpace(),
                contentConsumptionDbInfo.getTableName(),
   ...
   */
  @SuppressWarnings("unchecked")
  private void insertRecordIntoAssmntEvalTable(
      List<Map<String, Object>> assmntList, Map<String, Object> assmntReq) {
    for (Map<String, Object> map : assmntList) {
      try {
        ...
        if (null != resList && resList.size() > 0) {
          ...
          Double scoreFrmDbRes =
              (resMap.get(JsonKey.ASSESSMENT_SCORE)) != null
                  ? Double.parseDouble((String) resMap.get(JsonKey.ASSESSMENT_SCORE))
                  : 0d;
          Double scoreFrmReq =
              (map.get(JsonKey.ASSESSMENT_SCORE)) != null
          ...
          if (scoreFrmDbRes > scoreFrmReq) {
            Map<String, Object> reqMap = new HashMap<>();
            int attemptedCount =
                (resMap.get(JsonKey.ATTEMPTED_COUNT) != null)
            ...
            cassandraOperation.updateRecord(
                assmntEvalDbInfo.getKeySpace(), assmntEvalDbInfo.getTableName(), map);
          } else {
