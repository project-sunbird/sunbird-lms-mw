...
public class OrganisationManagementActor extends UntypedAbstractActor {
	...
	public void onReceive(Object message) throws Throwable {
		if (message instanceof Request) {
			try {
				...
				Util.initializeContext(actorMessage, JsonKey.ORGANISATION);
				// set request id fto thread loacl...
				ExecutionContext.setRequestId(actorMessage.getRequestId());
				if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.CREATE_ORG.getValue())) {
					createOrg(actorMessage);
				} else if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.UPDATE_ORG_STATUS.getValue())) {
				...
				} else if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.APPROVE_ORG.getValue())) {
				...
				} else if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.GET_ORG_TYPE_LIST.getValue())) {
					getOrgTypeList();
				} else if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.CREATE_ORG_TYPE.getValue())) {
					createOrgType(actorMessage);
				} else if (actorMessage.getOperation().equalsIgnoreCase(ActorOperations.UPDATE_ORG_TYPE.getValue())) {
	...
	private String validateOrgType(String orgType) {
		...
		if (!ProjectUtil.isStringNullOREmpty((String) DataCacheHandler.getOrgTypeMap().get(orgType.toLowerCase()))) {
