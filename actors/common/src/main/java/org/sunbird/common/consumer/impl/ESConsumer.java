    ...
    public void consume() {
        ...
        private void performJob() {
            ...
            if(JsonKey.INSERT.equalsIgnoreCase(eventMessage.getOperationType())) {
                ElasticSearchUtil.upsertData(ProjectUtil.EsIndex.sunbird.getIndexName(), eventMessage.getOpertaionType(), (String) eventMessage.getMessage().get(identiFierMap.get(eventMessage.getOperationOn())), eventMessage.getMessage());
            }else{
                if(updateMap.get(eventMessage.getOperationOn())) {
                    Map<String, Object> esMap = ElasticSearchUtil.getDataByIdentifier(ProjectUtil.EsIndex.sunbird.getIndexName(), eventMessage.getOpertaionType(), (String) eventMessage.getMessage().get(identiFierMap.get(eventMessage.getOperationOn())));
