...
public class SendOTPActor extends BaseActor {
  ...
  private void sendOTPViaEmail(String key, String otp, String otpType, String template) {
    ...
    ProjectLogger.log("SendOTPActor:sendOTP : Calling EmailServiceActor for Key = "+logMaskService.maskEmail(key),LoggerEnum.INFO.name());
  ...
  private void sendOTPViaSMS(String key, String otp, String template) {
    ...
    ProjectLogger.log("SendOTPActor:sendOTPViaSMS : Calling OTPUtil.sendOTPViaSMS for Key = "+logMaskService.maskPhone(key),LoggerEnum.INFO.name());
