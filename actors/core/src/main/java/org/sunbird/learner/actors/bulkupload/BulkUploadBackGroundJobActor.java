...
public class BulkUploadBackGroundJobActor extends BaseActor {
	...
	private void processBatchEnrollment(List<Map<String, Object>> jsonList, String processId) {
		...
		for (Map<String, Object> batchMap : jsonList) {
			...
			if (msg.equals(JsonKey.SUCCESS)) {
				List<Map<String, Object>> courseList = (List<Map<String, Object>>) courseBatchResult
	...
	private Boolean addUserCourses(String batchId, String courseId, String userId,
		...
		try {
			cassandraOperation.insertRecord(courseEnrollmentdbInfo.getKeySpace(), courseEnrollmentdbInfo.getTableName(),
					userCourses);
	...
	@SuppressWarnings("unchecked")
	private void processOrg(Map<String, Object> map, Map<String, Object> dataMap, List<Map<String, Object>> successList,
			List<Map<String, Object>> failureList, Map<String, String> channelToRootOrgCache) {
		...
		if (isNull(concurrentHashMap.get(JsonKey.ORGANISATION_NAME))
				|| ProjectUtil.isStringNullOREmpty((String) concurrentHashMap.get(JsonKey.ORGANISATION_NAME))) {
		...
		if (concurrentHashMap.containsKey(JsonKey.CONTACT_DETAILS)
				&& !ProjectUtil.isStringNullOREmpty((String) concurrentHashMap.get(JsonKey.CONTACT_DETAILS))) {
		...
		if (isRootOrg) {
			...
			if (isNotNull(esResult) && esResult.containsKey(JsonKey.CONTENT) && isNotNull(esResult.get(JsonKey.CONTENT))
				...
				try {
					...
					orgResponse.put(JsonKey.OPERATION, ActorOperations.INSERT_ORG_INFO_ELASTIC.getValue());
					ProjectLogger.log(
							"Calling background job to save org data into ES" + contentList.get(0).get(JsonKey.ID));
					Request request = new Request();
					request.setOperation(ActorOperations.INSERT_ORG_INFO_ELASTIC.getValue());
					request.getRequest().put(JsonKey.ORGANISATION, concurrentHashMap);
					tellToAnother(request);
			...
			if (concurrentHashMap.containsKey(JsonKey.CHANNEL)
				...
				String rootOrgId = Util.getRootOrgIdFromChannel((String) concurrentHashMap.get(JsonKey.PROVIDER));
	...
	private void processUserInfo(List<Map<String, Object>> dataMapList, String processId, String updatedBy) {
		...
		try {
			...
			ProjectLogger.log(
					"Exception occurred while encrypting success and failure result in bulk upload process : ", e1);
		}
		map.put(JsonKey.PROCESS_END_TIME, ProjectUtil.getFormattedDate());
		map.put(JsonKey.STATUS, ProjectUtil.BulkProcessStatus.COMPLETED.getValue());
	...
	private void processAuditLog(Map<String, Object> dataMap, String actorOperationType, String updatedBy,
			String objectType) {
	...
	private void insertRecordToUserExtTable(Map<String, Object> requestMap) {
		...
		if (requestMap.containsKey(JsonKey.PHONE)
			...
			if (null != (requestMap.get(JsonKey.PHONE_VERIFIED)) && (boolean) requestMap.get(JsonKey.PHONE_VERIFIED)) {
				map.put(JsonKey.IS_VERIFIED, true);
			}
	...
	private String generatePrimaryKey(Map<String, Object> req) {
		...
		return OneWayHashing.encryptVal(
				userId + JsonKey.PRIMARY_KEY_DELIMETER + courseId + JsonKey.PRIMARY_KEY_DELIMETER + batchId);
