...
public class CourseMetricsActorTest {
  ...
  private static void insertUserCoursesDataToES() {
    ...
    ElasticSearchUtil.createData(EsIndex.sunbird.getIndexName(), EsType.usercourses.getTypeName(),
        batchId + JsonKey.PRIMARY_KEY_DELIMETER + userId, userCoursesMap);
  }
  ...
  private static void insertOrgDataToES() {
    ...
    ElasticSearchUtil.createData(EsIndex.sunbird.getIndexName(), EsType.organisation.getTypeName(),
        batchId, orgMap);
  }
  ...
  public void testCourseConsumption() {
    ...
    Assert.assertEquals("mclr309f39",
        ((Map<String, Object>) data.get("course")).get(JsonKey.COURSE_ID));
