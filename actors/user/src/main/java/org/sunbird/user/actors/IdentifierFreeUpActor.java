...
public class IdentifierFreeUpActor extends BaseActor {
    ...
    private Response processUserAttribute(List<String> identifiers) {
        ...
        identifiers.stream().forEach(val -> {
    ...
    private void freeUpUser(String id, List<String> identifiers) {
        ...
        ProjectLogger.log(String.format("%s:%s:USER MAP SUCCESSFULLY UPDATED IN CASSANDRA. WITH VALUES  %s", this.getClass().getSimpleName(), "freeUpUser", Collections.singleton(userDbMap.toString())));
        saveUserDetailsToEs();
    }
