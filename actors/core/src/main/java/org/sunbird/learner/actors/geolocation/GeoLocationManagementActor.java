...
public class GeoLocationManagementActor extends BaseActor {
	...
	private void deleteGeoLocation(Request actorMessage) {
		...
		targetObject = TelemetryUtil.generateTargetObject(locationId, JsonKey.LOCATION, JsonKey.DELETE, null);
	...
	private void updateGeoLocation(Request actorMessage) {
		...
		targetObject = TelemetryUtil.generateTargetObject(locationId, JsonKey.LOCATION, JsonKey.UPDATE, null);
		TelemetryUtil.telemetryProcessingCall(actorMessage.getRequest(), targetObject, correlatedObject);

	}

	/**
	 * Get geo location on basis of type and id . type should be organisation or
	 * location .
	 * 
	 * @param actorMessage
	 */
	private void getGeoLocation(Request actorMessage) {
		...
		if (type.equalsIgnoreCase(JsonKey.ORGANISATION)) {
			...
			throw new ProjectCommonException(ResponseCode.invalidTypeValue.getErrorCode(),
					ResponseCode.invalidTypeValue.getErrorMessage(), ResponseCode.CLIENT_ERROR.getResponseCode());
		}
	}
	...
	private void createGeoLocation(Request actorMessage) {
		...
		List<Map<String, Object>> dataList = (List<Map<String, Object>>) actorMessage.getRequest().get(JsonKey.DATA);
