...
public class EmailServiceActor extends BaseActor {
	...
	private void sendMail(Request actorMessage) {
		...
		String name = "";
		Map<String, Object> request = (Map<String, Object>) actorMessage.getRequest().get(JsonKey.EMAIL_REQUEST);
		...
		if (!emails.isEmpty()) {
			...
			if (esResult.get(JsonKey.CONTENT) != null && !((List) esResult.get(JsonKey.CONTENT)).isEmpty()) {
				List<Map<String, Object>> esSource = (List<Map<String, Object>>) esResult.get(JsonKey.CONTENT);
		...
		SendMail.sendMail(emailIds.toArray(new String[emailIds.size()]), (String) request.get(JsonKey.SUBJECT),
				ProjectUtil.getContext(request), ProjectUtil.getTemplate(request));
	...
	private String getOrgName(String usrId) {
		String orgName = "";
		Map<String, Object> esUserResult = ElasticSearchUtil.getDataByIdentifier(EsIndex.sunbird.getIndexName(),
	...
	private Map<String, Object> getUserInfo(List<String> emails) {
		...
		return ElasticSearchUtil.complexSearch(searchDTO, EsIndex.sunbird.getIndexName(), EsType.user.getTypeName());
	}
